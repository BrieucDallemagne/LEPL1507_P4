...
def spherical_satellites_repartition_old(cities_coordinates, cities_weights, height=4, verbose=False):
    ...
    phi_values = np.linspace(0, 2 * np.pi, 20)[1:-1]

    # Create 2D arrays for theta and phi
    phi, theta = np.meshgrid(phi_values, theta_values)

    #Calculate x, y and z on a torus
    x_grid= (earth_radius + (height+earth_radius2) * np.cos(phi)) * np.cos(theta)
    y_grid= (earth_radius + (height+earth_radius2) * np.cos(phi)) * np.sin(theta)
    z_grid= (height+earth_radius2) * np.sin(phi)
...
def spherical_satellites_repartition(sph_cities_coords, cities_weights, height=4, verbose=False):
    ...
    x_grid = (sphere_center[0] + (earth_radius + (height+earth_radius2) * np.cos(phi)) * np.cos(theta)).flatten()
    y_grid = (sphere_center[1] + (earth_radius + (height+earth_radius2) * np.cos(phi)) * np.sin(theta)).flatten()
    z_grid = (sphere_center[2] + (height+earth_radius2) * np.sin(phi)).flatten()
    
    # Create a matrix for distances
    grid_points = np.column_stack((x_grid, y_grid, z_grid))
    distances_matrix = cdist(cities_coordinates, grid_points)
    sph_cities_coords_dephased = copy.copy(sph_cities_coords)
    sph_cities_coords_dephased[:, 0] += np.pi
    inv_squared_distances_matrix = fm.I_tore(sph_cities_coords_dephased,cities_coordinates,x_grid,y_grid,z_grid)#1 / (np.square(distances_matrix))    
    # Variables
    satellite_positions = cp.Variable(len(theta_values) * len(phi_values), boolean=True)
    ...
    # Constraints
    constraints = []
    min_intensity = fm.minimum_intensity2(height,fm.I) #fm.minimum_intensity2(height) 
